...
export const Avatar = forwardRef<HTMLDivElement, AvatarProps>((props, ref) => {
  const { className, sx, src, size = 'md', priority = false, isNextImage = true, ...rest } = props;

  return (
    <div
      ref={ref}
      className={clsx(avatarClasses.root, sx && configs(sx as object), avatar({ size }), className)}
      {...rest}
    >
      {isNextImage ? (
        <Image src={src} layout="fill" objectFit="cover" alt="User Icon" priority={priority} />
      ) : (
