...
import { snackbar, snackbarButton, snackbarContent, snackbarIcon } from './snackbar.css';
...
export const Snackbar = forwardRef<HTMLDivElement, SnackbarProps>((props, ref) => {
  ...
  return (
    <button className={snackbarButton()} onClick={onClose}>
      <div
        ref={ref}
        className={clsx(snackbarClasses.root, sx && configs(sx as object), snackbar(), className)}
        role="presentation"
        {...rest}
      >
        <Fade inProp={isOpen} timeout={defaultAnimationTimeout}>
          <div className={clsx(snackbarClasses.content, snackbarContent())}>
            {status === 'success' && (
              <MdTaskAlt className={clsx(snackbarClasses.icon, snackbarIcon({ status }))} />
            )}
            {status === 'error' && (
              <MdErrorOutline className={clsx(snackbarClasses.icon, snackbarIcon({ status }))} />
            )}
            {message}
          </div>
        </Fade>
